...
public class Trees_sorted_by_height {
    public static void main(String[] args) throws Exception {
        Configuration conf = new Configuration();
        String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
        if (otherArgs.length < 2) {
            System.err.println("Usage: Trees_sorted_by_height <in> [<in>...] <out>");
            System.exit(2);
        }
        Job job = Job.getInstance(conf, "Trees_sorted_by_height");
        job.setJarByClass(Trees_sorted_by_height.class);
