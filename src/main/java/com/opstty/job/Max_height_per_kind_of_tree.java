...
public class Max_height_per_kind_of_tree {
    public static void main(String[] args) throws Exception {
        Configuration conf = new Configuration();
        String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
        if (otherArgs.length < 2) {
            System.err.println("Usage: Max_height_per_kind_of_tree <in> [<in>...] <out>");
            System.exit(2);
        }
        Job job = Job.getInstance(conf, "Max_height_per_kind_of_tree");
        job.setJarByClass(Max_height_per_kind_of_tree.class);
